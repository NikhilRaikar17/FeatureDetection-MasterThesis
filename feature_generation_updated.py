...
def named_Entity(cluster):
    ...
    extractNNP(iob_tagged, nnp)

    extractOther(NNJJ, iob_tagged, noun_add, trigr)

    # Calculate Frequency
    frequencyNNP(NNP_Final, nnp)

    PatternExtract(NNJJ, features_list_aj_nn, features_list_big1, features_list_big2, features_list_nouns, noun_add,trigr)

    HandleTrigrams(feature_final, features_list_aj_nn, features_list_nouns)

    complete_set = FinalFeatureExtraction(complete_set, feature_final, features_list_nouns)

    AndOrExtract(NNP_Final, andor, complete_set)

    # To find Mandatory node.
    extractMandatory(cluster, complete_set)


def extractMandatory(cluster, complete_set):
    mand = []
    clu = cluster.split(".")
...
def AndOrExtract(NNP_Final, andor, complete_set):
    # # To Find AND and OR.
...
def FinalFeatureExtraction(complete_set, feature_final, features_list_nouns):
...
def HandleTrigrams(feature_final, features_list_aj_nn, features_list_nouns):
...
def PatternExtract(NNJJ, features_list_aj_nn, features_list_big1, features_list_big2, features_list_nouns, noun_add,trigr):
...
def frequencyNNP(NNP_Final, nnp):
    # For NNP.
...
def extractOther(NNJJ, iob_tagged, noun_add, trigr):
